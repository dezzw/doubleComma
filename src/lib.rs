...
fn command_plan(prefix: &[&str], args: &[OsString]) -> CommandPlan {
    let mut argv = argv(prefix);
    argv.extend(args.iter().cloned());
    CommandPlan::new(argv)
}

fn command_plan_with_path(prefix: &[&str], path: &Path, args: &[OsString]) -> CommandPlan {
    let mut argv = argv(prefix);
    argv.push(os(path));
    argv.extend(args.iter().cloned());
    CommandPlan::new(argv)
...
fn npm_run_script_plan(target: &str, args: &[OsString]) -> CommandPlan {
    let mut argv = argv(&["npm", "run", target]);
    if !args.is_empty() {
...
fn local_typescript_runner(project: &Project) -> Option<&'static str> {
    ["tsx", "ts-node"]
        .into_iter()
        .find(|name| has_local_node_bin(project, name))
}
...
fn build_node_file_plan(project: &Project, path: &Path, args: &[OsString]) -> Option<CommandPlan> {
    match path.extension().and_then(|extension| extension.to_str()) {
        Some("js" | "mjs" | "cjs") => Some(command_plan_with_path(&["node"], path, args)),
        Some("ts" | "tsx") => local_typescript_runner(project).map(|runner| {
            command_plan_with_path(&["npm", "exec", "--no", "--", runner], path, args)
        }),
        _ if is_executable_file(path) => Some(command_plan_with_path(&[], path, args)),
        _ => None,
    }
}
...
fn build_run_plan(
    ...
    match project.backend {
        ...
        Backend::Npm => {
            ...
            match intent {
                "test" => return Some(command_plan(&["npm", "test"], args)),
                "dev" | "build" | "fmt" | "lint" => {
                    return Some(npm_run_script_plan(intent, args));
                }
                _ => {}
...
mod tests {
    ...
    fn node_javascript_file_from_subdirectory_uses_absolute_path() {
        ...
        assert_eq!(plan.argv, vec![OsString::from("node"), os(&file)]);
    }
    ...
    fn uv_file_from_subdirectory_uses_absolute_path() {
        ...
        assert_eq!(
            plan.argv,
            vec![OsString::from("uv"), OsString::from("run"), os(&file)]
        );
    }
